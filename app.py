...
def projected_score(team_df, home, away, neutral: bool):
    ...
    h = tdf.loc[home]
    a = tdf.loc[away]
def get_team_long_name(schedule_df):
    """
    Map Team -> long display name using Schedule!AP:AQ (headers: Team, Name).
    Falls back to simple Team if not found.
    """
    # If the sheet already exposes columns named exactly 'Team' & 'Name' anywhere:
    ...
    if 'team' in cols and 'name' in cols:
        ...
        m = (schedule_df[[team_col, name_col]]
             .dropna()
...
def schedule_for_team(schedule_df, team):
    ...
    out = pd.DataFrame({
        ...
        'Opponent': sdf[opp_disp_col] if opp_disp_col in sdf else "",
...
def render_schedule_table(table_df, team_map_for_logo, on_click_query_builder):
    """
    table_df must have: Game, Date, Opponent, Opp Rank, Projected Spread, Win Probability (or numeric),
    Neutral (bool), _opp_team (optional key).
    on_click_query_builder(home_team, opp_team, neutral_bool) -> href string
    """
    view = table_df.copy()

    # Win prob display with inline bar
    def wp_cell(p):
        if pd.isna(p): return ""
        pct = int(round(float(p)*100))
        return f'''
        <div style="display:flex;align-items:center;gap:6px;">
          <div style="width:66px;border:1px solid #ddd;height:10px;border-radius:2px;overflow:hidden;">
            <div style="width:{pct}%;height:100%;background:#002060;"></div>
          </div>
          <span style="font-weight:600;font-size:11px;">{pct}%</span>
        </div>
        '''

    view['Win Probability'] = view['Win Probability (num)'].map(wp_cell).fillna(view['Win Probability'])

    # Make Opponent clickable → Comparison tab
    def linkify(row):
        opp_team_key = (row.get('_opp_team') or "").strip()
        # If we don't have a clean key, try to strip leading "vs/at " etc. from display text
        if not opp_team_key:
            disp = row['Opponent']
            guess = disp.replace('vs ', '').replace('at ', '').strip()
            opp_team_key = guess
        href = on_click_query_builder(opp_team_key, bool(row.get('Neutral', False)))
        return f'<a href="{href}" style="text-decoration:none;color:#0b3d91;">{row["Opponent"]}</a>'

    view['Opponent'] = view.apply(linkify, axis=1)

    # Keep/rename columns and render
    cols = ['Game','Date','Opponent','Opp Rank','Projected Spread','Win Probability']
    present = [c for c in cols if c in view.columns]
    styled = (view[present]
              .style.hide(axis='index')
              .set_table_styles([{'selector':'th',
                                  'props':'background:#001a57;color:white;font-weight:600;font-size:11px;'}])
             )
...
if tab_choice == "📊 Team Dashboards":
    ...
    # Build team frame once (for ranks + ratings + logo)
    team_frame = build_team_frame(df, metrics_df, logos_df)
    team_frame_idx = team_frame.set_index('Team', drop=False)

    all_teams = sorted(team_frame['Team'].tolist())
    default_team = st.session_state.get('selected_team', all_teams[0] if all_teams else "")
    if default_team not in all_teams:
        default_team = all_teams[0] if all_teams else ""

    selected_team = st.selectbox("Select a Team", options=all_teams, index=all_teams.index(default_team))
    st.session_state['selected_team'] = selected_team

    # Long team name from Schedule!AP:AQ
    team_long_map = get_team_long_name(schedule_df)
    team_long = team_long_map.get(selected_team, selected_team)

    # Logo + ranks + records
    row = team_frame_idx.loc[selected_team]
    logo_url = row.get('Logo', '')

    # Current record from df if available
    cur_w = int(df.loc[selected_team]['W']) if 'W' in df.columns and pd.notna(df.loc[selected_team]['W']) else 0
    cur_l = int(df.loc[selected_team]['L']) if 'L' in df.columns and pd.notna(df.loc[selected_team]['L']) else 0

    # Remaining expected record if present, else fall back to overall projection
    rem_w = df.loc[selected_team].get('Proj Rem W', np.nan) if selected_team in df.index else np.nan
    rem_l = df.loc[selected_team].get('Proj Rem L', np.nan) if selected_team in df.index else np.nan
    if (pd.isna(rem_w) or pd.isna(rem_l)):
        # try overall projection
        rem_w = df.loc[selected_team].get('Proj W', np.nan)
        rem_l = df.loc[selected_team].get('Proj L', np.nan)

    # Header card
    st.markdown(
        f"""
        <div style="display:flex;gap:12px;align-items:center;padding:8px 10px;border:1px solid #e6e8ee;border-radius:12px;background:#f8f9fb;">
          <img src="{logo_url}" alt="logo" style="width:56px;height:56px;object-fit:contain;border-radius:6px;">
          <div style="line-height:1.25;">
            <div style="font-weight:700;">{team_long}</div>
            <div>Rank: <b>{row['Pwr Rank']}</b> &nbsp; Off Rank: <b>{row['Off Rank']}</b> &nbsp; Def Rank: <b>{row['Def Rank']}</b></div>
            <div>Record: <b>{cur_w}-{cur_l}</b></div>
            <div>Expected Record: <b>{'' if pd.isna(rem_w) else f'{float(rem_w):.1f}'}-{'' if pd.isna(rem_l) else f'{float(rem_l):.1f}'}</b></div>
          </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    from urllib.parse import quote
    def comparison_link(opp_team_guess, neutral_bool):
        # If the opponent isn't recognized, still build the link using the display name
        opp = opp_team_guess or ""
        return (f"?selected_team={quote(selected_team)}"
                f"&home={quote(selected_team)}&away={quote(opp)}"
                f"&neutral={'1' if neutral_bool else '0'}#🤝%20Comparison")

    render_schedule_table(team_sched, team_logo_map, lambda opp, neu: comparison_link(opp, neu))

    # ---------------- Metrics mini-table ----------------
    st.markdown("#### Team Metrics")
    side = st.selectbox("Unit", ["Offensive", "Defensive"], key="td_unit")

    # Build a tidy list of metrics (labels → (col, higher_is_better))
    OFF_COLS = [
        ("Yards/Game", "Off. Yds/Game", True),
        ("Pass Yards/Game", "Off. Pass Yds/Game", True),
        ("Rush Yards/Game", "Off. Rush Yds/Game", True),
        ("Points/Game", "Off. Points/Game", True),
        ("Yards/Play", "Off. Yds/Play", True),
        ("Pass Yards/Play", "Off. Pass Yds/Play", True),
        ("Rush Yards/Play", "Off. Rush Yds/Play", True),
        ("Points/Play", "Off. Points/Play", True),
        ("EPA/Play", "Off. EPA/Play", True),
        ("Pass EPA/Play", "Off. Pass EPA/Play", True),
        ("Rush EPA/Play", "Off. Rush EPA/Play", True),
        ("Pts/Scoring Opp.", "Off. Points/Scoring Opp.", True),
        ("Success Rate", "Off. Success Rate", True),
        ("Pass Success Rate", "Off. Pass Success Rate", True),
        ("Rush Success Rate", "Off. Rush Success Rate", True),
        ("Explosiveness", "Off. Explosiveness", True),
        ("Pass Explosiveness", "Off. Pass Explosivenes", True),  # tolerate sheet spelling
        ("Rush Explosiveness", "Off. Rush Explosiveness", True),
    ]
    DEF_COLS = [
        ("Yards/Game", "Def. Yds/Game", False),
        ("Pass Yards/Game", "Def. Pass Yds/Game", False),
        ("Rush Yards/Game", "Def. Rush Yds/Game", False),
        ("Points/Game", "Def. Points/Game", False),
        ("Yards/Play", "Def. Yds/Play", False),
        ("Pass Yards/Play", "Def. Pass Yds/Play", False),
        ("Rush Yards/Play", "Def. Rush Yds/Play", False),
        ("Points/Play", "Def. Points/Play", False),
        ("EPA/Play", "Def. EPA/Play", False),
        ("Pass EPA/Play", "Def. Pass EPA/Play", False),
        ("Rush EPA/Play", "Def. Rush EPA/Play", False),
        ("Pts/Scoring Opp.", "Def. Points/Scoring Opp.", False),
        ("Success Rate", "Def. Success Rate", False),
        ("Pass Success Rate", "Def. Pass Success Rate", False),
        ("Rush Success Rate", "Def. Rush Success Rate", False),
        ("Explosiveness", "Def. Explosiveness", False),
        ("Pass Explosiveness", "Def. Pass Explosivenes", False),
        ("Rush Explosiveness", "Def. Rush Explosiveness", False),
    ]
    metric_list = OFF_COLS if side == "Offensive" else DEF_COLS

    mdf = metrics_df.copy()
    mdf.columns = mdf.columns.str.strip()
    mdf_idx = mdf.set_index('Team', drop=False)

    rows = []
    for label, col, higher_is_better in metric_list:
        if col not in mdf_idx.columns:
            continue
        series = pd.to_numeric(mdf_idx[col], errors='coerce')
        rk = series.rank(ascending=not higher_is_better, method='min').astype('Int64')
        val = series.get(selected_team, np.nan)
        rnk = rk.get(selected_team, pd.NA)
        # Format (value, rank) – rates as % with 1 decimal if it contains "Success Rate"
        if "Success Rate" in label:
            val_txt = "" if pd.isna(val) else f"{float(val):.1%}"
        else:
            val_txt = "" if pd.isna(val) else f"{float(val):.1f}"
        rows.append((label, val_txt, ("" if pd.isna(rnk) else f"{int(rnk)}")))
    mini = pd.DataFrame(rows, columns=["Metric", "Value", "Rank"]).reset_index(drop=True)

    # Match your Metrics tab look (dark header, compact)
    styled = (mini.style.hide(axis="index")
              .set_table_styles([{'selector':'th',
                                  'props':'background:#001a57;color:#fff;font-weight:600;font-size:11px;'},
...
# ----------------------------------------------------- COMPARISON TAB ------------------------------------------------
elif tab_choice == "🤝 Comparison":
    # --- Read query params from schedule links ---
    qp = st.query_params
    pre_home = qp.get("home", "")
    pre_away = qp.get("away", "")
    pre_neutral = qp.get("neutral", "0") == "1"

    st.markdown("## 🤝 Team Comparison")

    all_teams = sorted(df.index.tolist())

    # Use query param defaults if valid, else fall back
    home_default = all_teams.index(pre_home) if pre_home in all_teams else 0
    away_default = all_teams.index(pre_away) if pre_away in all_teams else 1

    col1, col2 = st.columns(2)
    with col1:
        team1 = st.selectbox("Home Team", all_teams, index=home_default, key="comp_home")
    with col2:
        team2 = st.selectbox("Away Team", all_teams, index=away_default, key="comp_away")

    neutral_site = st.checkbox("Neutral Site", value=pre_neutral, key="comp_neutral")
